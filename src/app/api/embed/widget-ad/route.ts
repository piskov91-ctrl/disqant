...
const EMBED_WIDGET_AD_CORS_HEADERS: Record<string, string> = {
...
function withEmbedCors(response: Response, allowOrigin: string): Response {
  ...
  for (const [k, v] of Object.entries(EMBED_WIDGET_AD_CORS_HEADERS)) {
    headers.set(k, v);
  }
...
/** Widget embed: promo content for generation loading overlay (valid client key required). */
