...
import { recordSubscriptionsFeedback } from "@/lib/subscriptionsFeedbackStore";
...
export async function POST(req: Request) {
  ...
  const text = [`Subscriptions page feedback`, `Rating (${rating}/5): ${stars}`, "", messageRaw].join("\n");

  const html = `
  <h2>Subscriptions — customer feedback</h2>
  ...
  const { data, error } = await resend.emails.send({
    from,
    to: [TO_EMAIL],
    subject: `Subscriptions feedback: ${stars} (${rating}/5)`,
    text,
    html,
  });
  ...
  if (error) {
    return Response.json(
      { error: "Could not send your feedback. Please try again or email us at support@fit-room.com." },
      { status: 502 },
    );
  }
  ...
  try {
    redisId = await recordSubscriptionsFeedback({ rating, message: messageRaw });
  } catch (storeErr: unknown) {
    console.error("[fit-room][subscriptions-feedback] Redis record failed after successful email", {
      message: storeErr instanceof Error ? storeErr.message : String(storeErr),
    });
  }

  return Response.json({ ok: true, emailId: data?.id ?? null, redisId });
}
