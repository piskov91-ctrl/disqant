(function () {
  ...
  function parseWidgetAdBannerSlides(data) {
    ...
    if (Array.isArray(data.banners)) {
      for (var i = 0; i < data.banners.length; i++) {
        ...
        if (!url) continue;
        var slide = { url: url, durationSec: clampWidgetAdBannerDuration(b.durationSec) };
        var linkUrl = String(b.linkUrl || "").trim();
        if (linkUrl && /^https?:\/\//i.test(linkUrl)) slide.linkUrl = linkUrl;
        out.push(slide);
      }
    }
  ...
  var MAIN_IMAGE_MIN_PX = 160;
  var THUMBNAIL_MAX_PX = 140;
  var THUMBNAIL_CLOSEST_SEL = [
    "[class*='thumbnail']",
    "[class*='thumbnails']",
    "[data-thumbnail]",
    "button.thumbnail",
  ...
  function isLikelyThumbnailImage(img) {
    if (!img) return true;
    var rect = img.getBoundingClientRect();
    if (rect.width <= 0 || rect.height <= 0) return true;
    if (rect.width <= THUMBNAIL_MAX_PX || rect.height <= THUMBNAIL_MAX_PX) return true;
    if (rect.width >= MAIN_IMAGE_MIN_PX && rect.height >= MAIN_IMAGE_MIN_PX) return false;

    var el = img;
    var hops = 0;
    while (el && hops < 5) {
      if (el.matches && el.matches(THUMBNAIL_CLOSEST_SEL)) return true;
      el = el.parentElement;
      hops++;
    }
    return false;
  ...
  function isPrimaryProductImageCandidate(img) {
    if (!img || img.tagName !== "IMG") return false;
    if (isWidgetModalImage(img)) return false;
    if (img.getAttribute(WIDGET_ATTR_SKIP) === "1") return false;
    ...
    if (img.closest && (img.closest("[hidden]") || img.closest('[aria-hidden="true"]'))) return false;
    if (isLikelyThumbnailImage(img)) return false;
    var rect = img.getBoundingClientRect();
    if (rect.width < MAIN_IMAGE_MIN_PX || rect.height < MAIN_IMAGE_MIN_PX) return false;
  ...
  function scorePrimaryProductImage(img) {
    var rect = img.getBoundingClientRect();
    var score = viewportVisibleArea(rect) || (rect.width * rect.height);
    if (img.closest && img.closest(PRODUCT_GALLERY_SEL)) score *= 2;
    if (img.closest && img.closest(ACTIVE_SLIDE_SEL)) score *= 4;
    return score;
  }
  ...
  function findPrimaryProductImage() {
    ...
    for (var i = 0; i < imgs.length; i++) {
      var img = imgs[i];
      if (!isPrimaryProductImageCandidate(img)) continue;
      ranked.push({ img: img, score: scorePrimaryProductImage(img) });
    }
    ranked.sort(function (a, b) { return b.score - a.score; });
    for (var r = 0; r < ranked.length; r++) {
      var candidate = ranked[r].img;
      if (shouldSkipAccessoryImage(candidate)) {
        candidate.setAttribute(WIDGET_ATTR_SKIP, "1");
        continue;
      }
      return candidate;
    }
  ...
  function isEligibleImage(img) {
    ...
    if (widgetBindMode() !== "all" && isLikelyThumbnailImage(img)) return false;
  ...
  function injectStyles() {
    ...
    var css = ""
      ...
      + ".dq-backdrop{position:fixed;inset:0;z-index:2147483646;background:rgba(15,23,42,.45);display:flex;align-items:center;justify-content:center;padding:8px;opacity:0;transition:opacity .18s ease;}"
      + ".dq-backdrop.dq-open{opacity:1;}"
      + ".dq-backdrop.dq-dismiss-locked{cursor:default;-webkit-user-select:none;user-select:none;}"
      + ".dq-modal{position:relative;width:min(720px,100%);min-height:0;height:min(calc(100dvh - 16px),calc(100vh - 16px));max-height:min(calc(100dvh - 16px),calc(100vh - 16px));background:#2c241f;border:1px solid rgba(198,167,125,.22);border-radius:20px;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,.45);display:flex;flex-direction:column;color:#f5ede4;transform:translateY(10px) scale(.985);opacity:0;transition:transform .18s ease, opacity .18s ease;}"
      ...
      + ".dq-body{flex:1 1 auto;min-height:0;padding:4px;display:flex;flex-direction:column;gap:4px;overflow:hidden;background:#2c241f;}"
      + ".dq-tips-block{flex-shrink:0;display:flex;flex-direction:column;gap:4px;padding:6px;}"
      ...
      + ".dq-stage-wow{position:absolute;top:max(8px,env(safe-area-inset-top,0px));left:0;right:0;z-index:6;width:max-content;max-width:calc(100% - 24px);margin:0 auto;padding:6px 14px;border-radius:999px;background:rgba(12,10,8,.72);backdrop-filter:blur(6px);pointer-events:none;}"
      + ".dq-stage-wow.is-on{display:block;animation:dq-wow-in .6s ease both;}"
      ...
      + ".dq-stage-actions{position:absolute;left:0;right:0;bottom:0;z-index:7;display:flex;flex-direction:column;align-items:center;gap:8px;padding:10px 12px max(12px,env(safe-area-inset-bottom,0px));background:linear-gradient(to top,rgba(12,10,8,.92) 0%,rgba(12,10,8,.5) 55%,transparent 100%);pointer-events:none;}"
      ...
      + ".dq-limit-banner{flex-shrink:0;display:none;align-items:flex-start;gap:14px;padding:16px 18px;border-radius:18px;"
      ...
      // Mobile tweaks (stage fills flex space — no fixed height override)
  ...
  function buildTryOnUI(opts) {
    ...
    var stageActions = document.createElement("div");
    stageActions.className = "dq-stage-actions";
    ...
    stageActions.appendChild(stageActionsRow);
    stageActions.appendChild(generateBtn);
    ...
    function setStageActionsVisible(visible) {
      stageActions.style.display = visible ? "flex" : "none";
    }
    ...
    function setStageAdSlide(slide) {
      ...
      if (linkUrl && /^https?:\/\//i.test(linkUrl)) {
        if (!existingLink) {
          ...
          existingLink.className = "dq-stage-ad-link";
          existingLink.target = "_blank";
          existingLink.rel = "noopener noreferrer";
          stageAdOverlay.appendChild(existingLink);
          existingLink.appendChild(stageAdImg);
        }
        existingLink.href = linkUrl;
        stageAdOverlay.classList.add("is-clickable");
      } else {
        if (existingLink) {
          stageAdOverlay.appendChild(stageAdImg);
          existingLink.parentNode.removeChild(existingLink);
        }
        stageAdOverlay.classList.remove("is-clickable");
      }
    }
    ...
    function setStageAdOverlayVisible(show) {
      ...
      if (!showingStageAdOverlay) stageAdOverlay.classList.remove("is-clickable");
    }
    ...
    function stopLoading(ok) {
      ...
      window.setTimeout(function () {
        ...
        if (!wow.classList.contains("is-on")) setStageActionsVisible(true);
      }, ok ? 450 : 0);
    }
  ...
  var SCAN_BATCH_ALL_MODE = 6;
  var scanBatchScheduled = false;
  var scanBatchRunning = false;
  var scanBatchRescanPending = false;
  var scanBatchImgs = null;
  ...
  function finishScanBatch() {
    ...
    scanBatchImgs = null;
    scanBatchIndex = 0;
    if (scanBatchRescanPending) {
      scanBatchRescanPending = false;
      scanAndBind();
    }
  }

  function runScanBatchStep() {
    scanBatchScheduled = false;
    if (!shouldBindOnThisPage()) {
      finishScanBatch();
      return;
    }
    ...
    if (!scanBatchImgs) {
      ...
      if (widgetBindMode() === "all") {
        scanBatchImgs = Array.prototype.slice.call(document.images || []);
      } else {
        var primary = findPrimaryProductImage();
        unbindAllExcept(primary);
        scanBatchImgs = primary ? [primary] : [];
      }
      scanBatchIndex = 0;
    }

    var end = Math.min(scanBatchIndex + SCAN_BATCH_ALL_MODE, scanBatchImgs.length);
    for (; scanBatchIndex < end; scanBatchIndex++) {
      processImageForBinding(scanBatchImgs[scanBatchIndex]);
    }

    if (scanBatchIndex < scanBatchImgs.length) {
      scanBatchScheduled = true;
      window.requestAnimationFrame(runScanBatchStep);
      return;
    }
  ...
  function scanAndBindBatched() {
    if (scanBatchRunning) {
      scanBatchRescanPending = true;
      return;
    }
    scanBatchRunning = true;
    scanBatchImgs = null;
    scanBatchIndex = 0;
    if (!scanBatchScheduled) {
      scanBatchScheduled = true;
      window.requestAnimationFrame(runScanBatchStep);
    }
  }
  ...
  var primaryRescanTimer = 0;
  var primaryRescanFollowupTimer = 0;

  function schedulePrimaryRescan() {
    if (widgetBindMode() === "all") return;
    if (!shouldBindOnThisPage()) return;
    if (primaryRescanTimer) window.clearTimeout(primaryRescanTimer);
    primaryRescanTimer = window.setTimeout(function () {
      primaryRescanTimer = 0;
      scanAndBind();
    }, 180);
    if (primaryRescanFollowupTimer) window.clearTimeout(primaryRescanFollowupTimer);
    primaryRescanFollowupTimer = window.setTimeout(function () {
      primaryRescanFollowupTimer = 0;
      scanAndBind();
    }, 450);
  }
  ...
  function boot() {
    ...
    document.addEventListener("click", function (e) {
      if (widgetBindMode() === "all") return;
      var t = e.target;
      if (t && t.closest && t.closest(".dq-backdrop")) return;
      schedulePrimaryRescan();
    }, true);
